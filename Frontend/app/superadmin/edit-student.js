...
export default function SuperadminEditStudent() {
  const insets = useSafeAreaInsets();
  const router = useRouter();
  const { studentData, course = 'computerScience' } = useLocalSearchParams();
  ...
  const submit = async () => {
    setSaving(true);
    try {
      await apiService.updateStudentBySuperadmin(parsed.id, {
        course: String(course),
        full_name: form.fullName,
        father_name: form.fatherName,
        address: form.address,
        past_school: form.pastSchool,
      ...
      Alert.alert('Error', error?.error || error?.message || 'Update failed');
    } finally {
      setSaving(false);
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.content}>
        {[
          ...
          <View key={key} style={{ marginBottom: 12 }}>
            ...
            <TextInput style={styles.input} value={form[key]} onChangeText={(v) => setField(key, v)} />
          </View>
        ))}
